    ...
    <form action="register.php" method="post" onsubmit="return validateForm()">
      ...
      <?php
            ...
            if (isset($_GET['error']) && $_GET['error'] == 'invalid_credentials') {
              echo '<span style="color:red; display:flex;">Username Exist already, Use another username.</span>';
            } ?>
      ...
      <input type="password" id="password" name="password" pattern=".{8,}" oninput="validatePassword()">
      
      <!-- confirm pass -->
      <div class="stick"><span class="must-fill">*</span><label for="confirm_password">Confirm Password:</label></div>
      <input type="password" id="confirm_password" name="confirm_password" pattern=".{8,}" oninput="validatePassword()"><br>
      ...
      <div>
        <label for="sign-in" class="sign-inLable">If you already have account then</label><br>
         <input class="sign-in" type="button" onclick="document.location='loginPage.html'" value="Sign-in">
       </div>
    </form>
  </div>
